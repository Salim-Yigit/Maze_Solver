...
int findExit(int matris[70][70],int visited[70][70],int i,int j,int *puan,struct Stack *stack,char maze[70][70]);
...
int main() { 
	...
	//printMatris(visited,&n,&m);
	findExit(matris,visited,i,j,&puan,stack,maze); 
	return 0;
}  
...
int findExit(int matris[70][70],int visited[70][70],int i,int j,int *puan,struct Stack *stack,char maze[70][70]) {  
	printf("\n puaniniz:%d\n",*puan); 
	printMaze(maze,31,43,i,j,1);
	usleep(300000);
	if(matris[i+1][j]==1 && visited[i+1][j]==0) {  	
		push(stack,1); 
		//printf("Asagi gitti\n");
		visited[i+1][j]=1;
		return findExit(matris,visited,i+1,j,puan,stack,maze);
		...
		*puan=*puan+10;  
		...
		visited[i][j+1]=1; 
		push(stack,3);
		return findExit(matris,visited,i,j+1,puan,stack,maze);
